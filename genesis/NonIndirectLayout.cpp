...
namespace genesis
{
   ...
   void NonIndirectLayout::setupDescriptorPool(const VulkanGltfModel* model)
   {
      ...
      poolSizes.push_back(genesis::vkInitializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, (uint32_t)textures.size()));
   ...
   void NonIndirectLayout::setupDescriptorSetLayout(const VulkanGltfModel* model)
   {
      ...
      setBindings.push_back(genesis::vkInitializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, bindingIndex++));
   ...
   void NonIndirectLayout::updateDescriptorSets(const VulkanGltfModel* model)
   {
      ...
      {
         const VkDescriptorSetAllocateInfo descriptorSetAllocateInfo = genesis::vkInitializers::descriptorSetAllocateInfo(_descriptorPool, &_descriptorSetLayout, 1);
         ...
         {
         genesis::vkInitializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &textures[i]->descriptor())
         };
